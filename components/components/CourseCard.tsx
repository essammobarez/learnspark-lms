    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col h-full transition-all duration-300 ease-in-out hover:shadow-xl hover:scale-[1.03]">
      <Link to={`/courses/${course.id}`} className="block">
        <img 
          ...
          alt={course.title} 
          onError={(e) => {
            // If the primary imageUrl fails, and it wasn't already the placeholder, try the placeholder
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="flex items-center mb-3">
          ...
          <span className="text-xs text-gray-500 ml-2">({(course.enrollmentCount ?? 0).toLocaleString()} students)</span>
        </div>
