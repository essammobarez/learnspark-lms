...
import { TableCellsIcon, AcademicCapIcon, ArrowUturnLeftIcon } from '@heroicons/react/24/outline';
...
const StudentReportsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAttempts = async () => {
      if (user) {
        ...
        try {
          const attempts = await mockApiService.getQuizAttemptsForUser(user.id);
          setQuizAttempts(attempts);
        } catch (e) {
          console.error("Failed to fetch quiz attempts:", e);
          setError("Could not load your quiz reports. Please try again later.");
        } finally {
          setIsLoading(false);
        }
      } else {
  ...
  if (isLoading) {
    return ( <div className="text-center py-10"><LoadingSpinner /> <p className="mt-3 text-gray-600 dark:text-gray-400">Loading your quiz reports...</p></div> );
  }
  ...
  if (!user) {
    return (
      <div className="text-center py-10 bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl transition-colors duration-300 ease-in-out">
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Please log in to view your quiz reports.</p>
  ...
  if (error) {
    return (
      <div className="text-center py-10 bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl">
        <AcademicCapIcon className="mx-auto h-16 w-16 text-red-400 dark:text-red-500 mb-4" />
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-center pb-4 border-b border-gray-200 dark:border-gray-700">
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-gray-900 dark:text-white">My Quiz Reports</h1>
      ...
      {quizAttempts.length > 0 ? (
        ...
        <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 transition-colors duration-300 ease-in-out">
          ...
          <h2 className="mt-2 text-2xl font-semibold text-gray-900 dark:text-white">No Quiz Attempts Yet</h2>
          <p className="mt-2 text-md text-gray-600 dark:text-gray-400">You haven't taken any quizzes. Explore your courses and test your knowledge!</p>
