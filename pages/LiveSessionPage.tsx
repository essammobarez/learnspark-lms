...
const LiveSessionPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCourseDetails = async () => {
      if (!courseId || !user) { setMediaError("Course or user information is missing."); setInitialLoading(false); return; }
      setInitialLoading(true);
      try {
        const fetchedCourse = await mockApiService.getCourseById(courseId);
        if (!fetchedCourse) setMediaError("Course not found."); else setCourse(fetchedCourse);
      } catch (e) { setMediaError("Failed to load course details."); console.error(e); }
  ...
  useEffect(() => {
    if (!user || !hasMadeInitialChoice) return;
    const manageStream = async () => {
      setIsLoadingMedia(true); setMediaError(null);
      ...
      try {
        ...
        if (desiredVideoStreamState && videoTracks.length > 0) { videoTracks.forEach(track => track.enabled = true); setIsCameraEffectivelyOff(false); } 
        else { videoTracks.forEach(track => track.enabled = false); setIsCameraEffectivelyOff(true);
          if (desiredVideoStreamState && videoTracks.length === 0) console.warn("Video stream desired, but no video tracks found.");
        }
        newStream.getAudioTracks().forEach(track => track.enabled = !micMutedRef.current);
        if (desiredVideoStreamState && newStream.getVideoTracks().some(t=>t.enabled)) setMockParticipantStreams([newStream, newStream]); 
        else { const audioOnlyStream = new MediaStream(newStream.getAudioTracks()); setMockParticipantStreams([audioOnlyStream, audioOnlyStream]); }
      } catch (err) {
        ...
        setMediaError(specificError); setLocalStream(null); setIsCameraEffectivelyOff(true);
      } finally { setIsLoadingMedia(false); }
    };
    manageStream();
    return () => { if (localStream) localStream.getTracks().forEach(track => track.stop()); };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user, hasMadeInitialChoice, desiredVideoStreamState]); 
  ...
  const handleLeaveSession = () => {
    ...
    navigate(ROUTES.COURSE_DETAIL.replace(':courseId', courseId || '')); // Updated for v6+
  };
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-gray-800 dark:bg-gray-900 text-white flex flex-col p-2 sm:p-4 transition-colors duration-300 ease-in-out">
      ...
      <main className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-4">
        ...
        <div className="grid grid-rows-3 gap-2 h-[30vh] sm:h-[35vh] md:h-auto md:flex md:flex-col">
          {[...Array(3)].map((_, index) => ( // Simulating 3 participant slots
