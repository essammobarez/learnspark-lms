...
import { useParams, Link, useNavigate, useLocation } from 'react-router-dom'; // Updated for v6+: useNavigate instead of useHistory
...
import { PlayCircleIcon, DocumentTextIcon, PresentationChartBarIcon, PuzzlePieceIcon, PlusCircleIcon, VideoCameraIcon, PencilSquareIcon, PresentationChartLineIcon, StarIcon as StarSolidIcon } from '@heroicons/react/24/solid'; // Using solid for filled stars
...
const CourseDetailPage: React.FC = () => {
  ...
  const location = useLocation(); // Added for use in login redirect state
  ...
  useEffect(() => {
    const fetchCourseDetails = async () => {
      ...
      try {
        ...
        if (fetchedCourse && fetchedCourse.quizIds) {
          const fetchedQuizzesPromises = fetchedCourse.quizIds.map(id => mockApiService.getQuizById(id));
          const fetchedQuizzesResults = await Promise.all(fetchedQuizzesPromises);
          setQuizzes(fetchedQuizzesResults.filter(q => q !== null) as Quiz[]);
        } else if (!fetchedCourse) {
          setError("Course not found.");
        }
        
        if (user && fetchedCourse) {
          const enrolledCourses = await mockApiService.getEnrolledCourses(user.id);
  ...
  const handleEnroll = async () => {
    ...
    const success = await mockApiService.enrollInCourse(user.id, course.id);
    if (success) {
      setIsEnrolled(true);
            ...
            {isAuthenticated && user?.role === UserRole.STUDENT && isEnrolled && (
               <Link to={ROUTES.LIVE_SESSION.replace(':courseId', course.id)}>
                <Button variant="success" size="md" className="flex items-center">
                  <VideoCameraIcon className="w-5 h-5 mr-2" /> Join Live Session
                </Button>
              </Link>
            )}
          </div>
                ...
                {course.lessons.map((lesson, index) => (
                  <li key={lesson.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:shadow-lg dark:hover:bg-gray-700/50 transition-all duration-200 flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 capitalize">{lesson.type} - placeholder content</p>
                    </div>
                  </li>
                ))}
              </ul>
                  ...
                  <li key={quiz.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:shadow-lg dark:hover:bg-gray-700/50 transition-all duration-200">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 w-full sm:w-auto">
                          {(isEnrolled || isInstructorOwner) && quiz.questions.length > 0 && (
                          ...
                          {isInstructorOwner && quiz.questions.length > 0 && (
                    ...
                    {quiz.questions.length === 0 && <span className="text-sm text-gray-500 dark:text-gray-400 mt-2 block italic">This quiz currently has no questions.</span>}
                  </li>
                ))}
              </ul>
            ) : (
        ...
        <aside className="lg:col-span-4">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl sticky top-24 transition-colors duration-300 ease-in-out">
            ...
            <div className="mt-6 space-y-3 text-sm text-gray-700 dark:text-gray-300">
              ...
              <p><strong>Lessons:</strong> <span className="text-gray-600 dark:text-gray-400">{course.lessons.length}</span></p>
