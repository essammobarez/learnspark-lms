  ...
  if (!isAuthenticated || !user || user.role !== UserRole.INSTRUCTOR) {
    return (
       <div className="container mx-auto p-6 text-center">
        ...
        <p className="text-gray-600 mt-2">You must be an instructor to create a course.</p>
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-center">
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-gray-900">Create New Course</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl space-y-6 transition-colors duration-300 ease-in-out">
        ...
        <div>
          ...
          <textarea
            ...
            rows={4}
            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm bg-white text-gray-900 transition-colors duration-300 ease-in-out"
            required
        ...
        <div className="border-t border-gray-200 pt-6">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">Add Lessons</h2>
          {lessons.length > 0 && (
            <div className="mb-6 space-y-3">
                <h3 className="text-md font-medium text-gray-700">Added Lessons ({lessons.length}):</h3>
                <ul className="border border-gray-200 rounded-md shadow-sm divide-y divide-gray-200">
                    {lessons.map((l, i) => (
                    <li key={l.id} className="px-4 py-3 flex justify-between items-center text-sm hover:bg-gray-50 transition-colors">
                        <span className="text-gray-800">{i+1}. {l.title} <span className="text-xs text-gray-500">({l.type})</span></span>
          ...
          <div className="space-y-4 p-4 border border-gray-200 rounded-lg bg-gray-50">
            <h3 className="text-lg font-medium text-gray-700">New Lesson Details:</h3>
