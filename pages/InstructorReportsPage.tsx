...
import { QuizAttempt, Quiz, Course, UserRole } from '../types';
...
import { TableCellsIcon, DocumentChartBarIcon, AcademicCapIcon, FunnelIcon, ArrowUturnLeftIcon } from '@heroicons/react/24/outline';
    ...
    const fetchInitialData = async () => {
      if (user && user.role === UserRole.INSTRUCTOR) {
        ...
        try {
          ...
          const quizzesPromises = courses.flatMap(course => course.quizIds.map(quizId => mockApiService.getQuizById(quizId)));
          const quizzesResults = await Promise.all(quizzesPromises);
          const validQuizzes = quizzesResults.filter(q => q !== null) as Quiz[];
          setInstructorQuizzes(validQuizzes);
          const attempts = await mockApiService.getQuizAttemptsForInstructorQuizzes(user.id);
          setQuizAttempts(attempts);
        } catch (e) { console.error("Failed to fetch instructor reports data:", e); setError("Could not load report data. Please try again later.");
  ...
  if (!user || user.role !== UserRole.INSTRUCTOR) {
    return (
      <div className="text-center py-10 bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl transition-colors duration-300 ease-in-out">
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">You must be an instructor to view these reports.</p>
  ...
  if (error) {
    return (
      <div className="text-center py-10 bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl">
        <DocumentChartBarIcon className="mx-auto h-16 w-16 text-red-400 dark:text-red-500 mb-4" />
        <h2 className="mt-2 text-xl font-medium text-red-700 dark:text-red-400">Error Loading Reports</h2>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{error}</p>
        <div className="mt-6"> <Button variant="primary" onClick={() => window.location.reload()}>Try Again</Button> </div>
      </div> );
  }
      ...
      {filteredAttempts.length > 0 ? (
        ...
        <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 transition-colors duration-300 ease-in-out">
          ...
          <p className="mt-2 text-md text-gray-600 dark:text-gray-400">
            {selectedQuizId === 'all' ? "No students have taken any of your quizzes yet." : "No attempts found for the selected quiz."}
          </p>
           {instructorQuizzes.length === 0 && <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">You haven't created any quizzes yet.</p>}
